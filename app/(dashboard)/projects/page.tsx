...
import { projects, projectMembers, tasks } from "@/src/db/schema";
...
export default async function ProjectsPage() {
  ...
  // Fetch all projects with member count and task statistics
  ...
  const activeProjects = projectsWithData.filter(
    (p) => p.status === "ACTIVE"
  ).length;
  const totalTasks = projectsWithData.reduce((acc, p) => acc + p.tasksCount, 0);
  const completedTasks = projectsWithData.reduce(
    (acc, p) => acc + p.completedTasks,
    0
  );
